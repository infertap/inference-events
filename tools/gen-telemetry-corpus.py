#!/usr/bin/env python3
"""Generate telemetry transduction cases from the specification.

Each case checks field mapping, omission, identity scope, or event order. Expected
records are computed independently of the producer implementation.
"""
...
# Epoch milliseconds expose timestamp unit conversion errors.
T0 = 1785153670000.0
...
FIXTURES = [
    (
        ...
        [
            stored("a", H1, T0, seq=41),
            stored("a", H2, T0, parent_hash=H1, seq=41),
    ...
    (
        ...
        [
            stored("a", H1, T0),
            removed("a", H1, T0 + 100.0),
            stored("a", H1, T0 + 200.0),
        ],
    ),
    (
    ...
    (
        ...
        [
            stored("a", H1, T0, dp_rank=0),
            stored("a", H2, T0 + 10.0, dp_rank=1),
