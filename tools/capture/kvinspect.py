...
def hexify(v, cap=64):
    ...
    if isinstance(v, dict):
        return {k: hexify(x, cap) for k, x in v.items()}
    if isinstance(v, list):
        return [hexify(x, cap) for x in v[:4]] + (
...
for line in open(a.path):
    ...
    for ev in events:
        ...
        if isinstance(ev, dict):
            ...
            for k, v in ev.items():
                shapes[key][str(k)].add(d(v))
        elif isinstance(ev, (list, tuple)):
            key = f"array<{ev[0] if ev and isinstance(ev[0], str) else '?'}>"
            for i, v in enumerate(ev):
                shapes[key][f"[{i}]"].add(d(v))
        else:
            key = d(ev)
        counts[key] += 1
        if len(samples[key]) < a.samples:
            samples[key].append(hexify(ev))
