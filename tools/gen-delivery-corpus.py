#!/usr/bin/env python3
"""Generate delivery records and a sealed segment from the delivery specification.

The segment includes a header, lifecycle records, cache events, and endpoint statistics.
Its Parquet twin covers encoding equivalence.
"""
...
def heartbeat(at_ms):
    """Exercise nested endpoint columns with optional fields present and absent."""
    ...
    r["endpoints"] = [
        {
            ...
            "msgs_seen": 3,
            "dropped": 0,
            "last_msg_at_ms": at_ms - 5.0,
            "publisher_restarts": 0,
            "topic": "kv@pod-a@model-x",
        },
        {
            "source": "i0",
...
def main():
    cases = [
        ...
        },
        {
            ...
            "expect": segments_dropped(
                11, 14, T0 + 3000.0, incarnation="1785153600000-1817"
            ),
        },
    ]
    ...
    segment = {
        ...
        "lines": [
            segment_open(0, T0),
            cache_record(
                "store", "8f2b1c04a7d93e15", T0 + 10.0, n_tokens=16, tier="gpu"
            ),
            cache_record(
                "store", "b3e77a190c4f2d68", T0 + 20.0, n_tokens=16, tier="gpu"
            ),
            heartbeat(T0 + 25.0),
            cache_record("evict", "8f2b1c04a7d93e15", T0 + 30.0),
            {
                **cache_record(
                    "store", "c41d9f8a2b6e0357", T0 + 40.0, n_tokens=16, tier="gpu"
                ),
                **i1,
                "content_id": "11111111111111111111",
                "parent_id": "8f2b1c04a7d93e15",
    ...
    recovering = {
        ...
        "lines": [
            ...
            cache_record(
                "store", "8f2b1c04a7d93e15", T0 + 230_010.0, n_tokens=16, tier="gpu"
            ),
        ],
    }
